...
password = "liulijie123"

sem_num     = param['sem_num'] # example: "2015-2016-1"
course_id   = param['course_id'] # example: "40231162"
sub_id      = param['sub_id'] # example: "0"
freq        = param['freq']  # freq(second)
course_type = param['course_type'] # example: rx, ty, bx, cx, xx

csn = {}
csn["rx"] = 'p_rx_id'  # course type(rx, ty, bx, cx, xx).
csn["bx"] = 'p_bxk_id'
csn["ty"] = 'p_rxTy_id'
csn["xx"] = 'p_xxk_id'
csn["cx"] = 'p_cxk_id';

captcha = 'http://zhjwxk.cic.tsinghua.edu.cn/login-jcaptcah.jpg?captchaflag=login1'  # get the image
...
def getCaptcha(s):
    response = s.get(captcha, stream=True)  # get the image
...
while True:
    ...
    try:
        data = s.get("http://zhjwxk.cic.tsinghua.edu.cn/xkYjs.vxkYjsXkbBs.do?m=xkqkSearch&p_xnxq=%s" % (sem_num)).text
        html = lxml.html.fromstring(data)
        formV = extractForm(html.forms[0])
        formV['p_kch'] = course_id  # ke cheng hao course id
        formV['p_kxh'] = sub_id  # ke xu hao course id
    ...
    while True:
        try:
            data = s.post('http://zhjwxk.cic.tsinghua.edu.cn/xkYjs.vxkYjsJxjhBs.do', data=formV).text
            # html = lxml.html.fromstring(data)
            # formV = extractForm(html.forms[0])
            ...
            for td in tbRes.cssselect('td'):
                ...
                if ind == 5:
                    ...
                    break
            print str(left)
            if str(left) != "0" and int(left) != 0:
                html = lxml.html.fromstring(data)
                formV = extractForm(html.forms[0])
                formV['m'] = "saveXwKc"  # ""save%sKc" % course_type.title()
                formV['p_xwk_id'] = "%s;%s;%s;" % (sem_num, course_id, sub_id)
                # formV[csn[course_type]] = "%s;%s;%s;" % (sem_num, course_id, sub_id)
                data = s.post('http://zhjwxk.cic.tsinghua.edu.cn/xkYjs.vxkYjsXkbBs.do', data=formV).text
